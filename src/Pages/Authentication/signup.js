...
const Signup = () => {
    ...
    const submitHandler = ()=>{
        ...
        if(!userDetails.firstName || !userDetails.lastName || !userDetails.email || !userDetails.password || !userDetails.confirmPassword || !userDetails.terms){
            ...
            setError({isError: true, text: 'Password and confirm password do not match'})
        }else{
            signUp(userDetails)
    ...
    <div className="wrapper" style={{marginLeft: '30rem'}}>
        ...
        <form onSubmit={(e)=>{e.preventDefault()}}>
            <div className="input-box">
                <input type="text" placeholder="Enter your first name" value={userDetails.firstName} onChange={(e)=>setUserDetails({...userDetails, firstName: e.target.value})}  />
            </div>
            <div className="input-box">
                <input type="text" placeholder="Enter your last name" value={userDetails.lastName} onChange={(e)=>setUserDetails({...userDetails, lastName: e.target.value})} />
            </div>
            <div className="input-box">
            ...
            <div className="input-box">
                <input type="password" placeholder="Confirm password" value={userDetails.confirmPassword} onChange={(e)=>setUserDetails({...userDetails, confirmPassword: e.target.value})} />
            </div>
